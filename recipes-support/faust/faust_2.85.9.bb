...
# CMakeLists.txt is in build/, but S stays at the root: LIC_FILES_CHKSUM,
# patchdir and -ffile-prefix-map are all relative to it.
S = "${UNPACKDIR}/${BP}"
...
# Flags follow score's Flatpak manifest. USE_LLVM_CONFIG=OFF because OE's
# llvm-config cross wrapper emits nothing without NEXT_LLVM_CONFIG.
EXTRA_OECMAKE = " \
...
# The architecture tree (43 MB, with prebuilt arm/ios libsndfile) and the 114
# faust2* wrappers drive the faust CLI, which INCLUDE_EXECUTABLE=0 skips.
# Removing datadir also leaves it to faustlibraries.
do_install:append() {
