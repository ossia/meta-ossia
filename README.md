...
By default score is built from the **release source archive**, not a git
checkout: score has 39 submodules, libossia 44 more, and `ci/common.deps.sh`
clones ~20 addon repositories separately, so a `gitsm://` fetch of score alone
would silently produce a build with no addons. The archive from
`ci/tarball.build.sh` contains all of it and is published on every release.

To build a local checkout instead, set in `local.conf`:

```
SCORE_SRC_ROOT = "/path/to/score"
```

`ossia-score_git.bb` then takes over (it skips itself when `SCORE_SRC_ROOT` is
unset). Submodules and addons must already be present — `git submodule update
--init --recursive` plus `ci/common.deps.sh LINUX` — since `externalsrc` has no
